...
class PPI(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            self.memory.create_tensor(name="residual_actions", size=self.action_space, dtype=torch.float32)
            ...
            # tensors sampled during training
            self._tensors_names = ["student_obs", "teacher_obs", "actions", "teacher_actions", "rewards", "terminated", "log_prob", "lifted_now", "values", "returns", "advantages"]
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        actions, residual_actions, log_prob, outputs = self.policy.act({"states": self._state_preprocessor(states)}, role="policy")
        self._current_log_prob = log_prob

        return actions, residual_actions, log_prob, outputs
    ...
    def record_transition(self,
        ...
        :type timesteps: int
        """
        # super().record_transition(student_obs, actions, rewards, next_states, terminated, truncated, infos, timestep, timesteps)
        ...
        if self.memory is not None:
            ...
            # compute values
            values, _, _ = self.value.act({"states": self._state_preprocessor(student_obs)}, role="value")
            ...
            self.memory.add_samples(student_obs=student_obs, teacher_obs=teacher_obs, actions=actions, residual_actions=residual_actions, teacher_actions=teacher_actions, rewards=rewards, next_states=next_states,
                                    terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values, **rnn_states)
            for memory in self.secondary_memories:
                memory.add_samples(student_obs=student_obs, teacher_obs=teacher_obs, actions=actions, residual_actions=residual_actions, teacher_actions=teacher_actions, rewards=rewards, next_states=next_states,
                                   terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values, **rnn_states)
    ...
    ####################################kivy###################################### 
    def set_requires_grad(self, model, include=None, exclude=None):
        for name, param in model.named_parameters():
            if include is not None:
                param.requires_grad = any(key in name for key in include)
            elif exclude is not None:
                param.requires_grad = not any(key in name for key in exclude)
            else:
                param.requires_grad = True  # default case: all trainable
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if not self._rollout % self._rollouts and timestep >= self._learning_starts:
            self.set_mode("train")
            # if timestep < 210000:
            #     self.set_requires_grad(self.policy, exclude=["residual_mlp"])
            #     self._update(timestep, timesteps)
            #     self.set_requires_grad(self.policy)
            # else:
            self.set_requires_grad(self.policy, include=["residual_mlp"])
            self._update(timestep, timesteps)
            self.set_requires_grad(self.policy)
            self.set_mode("eval")
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        # Determine dynamic loss weights based on global step
        if timestep < 20000:
            im_weight = 1.0
            rl_weight = 0.0001
        elif timestep < 40000:
        ...
        for epoch in range(self._learning_epochs):
            ...
            # mini-batches loop
            # for sampled_student_obs, sampled_actions, sampled_log_prob, sampled_values, sampled_returns, sampled_advantages in sampled_batches:
            for sampled_student_obs, sampled_teacher_obs, sampled_actions, sampled_teacher_actions, sampled_rewards, sampled_terminated, sampled_log_prob, sampled_lifted_now, sampled_values, sampled_returns, sampled_advantages in sampled_batches:
                ...
                try:
                    _, _, next_log_prob, _ = self.policy.act({"states": sampled_student_obs, "taken_actions": sampled_actions}, role="policy")
                except:
                ...
                policy_loss_record = policy_loss
                policy_loss = rl_weight * policy_loss

                # compute value loss
                predicted_values, _, _ = self.value.act({"states": sampled_student_obs}, role="value")
                ...
                student_actions, residual_actions, _, _= self.policy.act({"states": sampled_student_obs, **rnn_policy}, role="policy")
                if self._fixed_base:
                    ...
                    imitation_loss = F.mse_loss(student_actions, sampled_teacher_actions)
                    residual_loss = F.mse_loss(residual_actions, sampled_teacher_actions - sampled_actions)
                    dagger_loss = imitation_loss 
                    dagger_loss = im_weight * imitation_loss 
                # print(f"dagger_loss: {dagger_loss.item()}")
                print(f"imitation_loss: {imitation_loss.item()}")
                #######################################imitation loss####################################
                
                policy_loss += dagger_loss
                ...
                cumulative_policy_loss += policy_loss_record.item()
                cumulative_dagger_loss += imitation_loss.item()
