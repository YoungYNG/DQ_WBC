...
class DAgger_RNN(Agent):
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        actions, residual_actions, outputs = self.policy.act({"states": states, **rnn}, role="policy")
        # if log_prob is None: 
        # log_prob = 0
        self._current_log_prob = 0
        
        if self._rnn:
            self._rnn_final_states["policy"] = outputs.get("rnn", [])
            self._rnn_initial_states["policy"] = self._rnn_final_states["policy"]
        
        return actions, residual_actions, outputs
    
    def record_transition(self, student_obs: torch.Tensor, teacher_obs, actions: torch.Tensor, residual_actions: torch.Tensor, teacher_actions: torch.Tensor, rewards: torch.Tensor, next_states: torch.Tensor, terminated: torch.Tensor, truncated: torch.Tensor, infos: Any, timestep: int, timesteps: int) -> None:
        ...
        if self.memory is not None:
            ...
            self.memory.add_samples(student_obs=student_obs, teacher_obs=teacher_obs, actions=actions, residual_actions=residual_actions, teacher_actions=teacher_actions, rewards=rewards,
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if not self._rollout % self._rollouts and timestep >= self._learning_starts:
            ...
            if timestep < 30:
                ...
                self._update_residual(timestep, timesteps)
                # self.set_requires_grad(self.policy)
            self.set_mode("eval")
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for epoch in range(self._learning_epochs):
            ...
            for i, (sampled_student_obs, sampled_teacher_obs, sampled_actions, sampled_teacher_actions, sampled_rewards, sampled_terminated, sampled_log_prob) in enumerate(sampled_batches):
                ...
                # compute policy loss
                student_actions, residual_actions, _= self.policy.act({"states": sampled_student_obs, **rnn_policy}, role="policy")
                ...
                # print(f"residual_loss: {residual_loss.item()}")
                # optimization step
        ...
        self.track_data("Loss / Residual loss", residual_loss / (self._learning_epochs * self._mini_batches))        
        if self._entropy_loss_scale:
    ...
    def _update_residual(self, timestep: int, timesteps: int) -> None:
        """Algorithm's main update step
        ...
        for epoch in range(self._learning_epochs):
            ...
            for i, (sampled_student_obs, sampled_teacher_obs, sampled_actions, sampled_teacher_actions, sampled_rewards, sampled_terminated, sampled_log_prob) in enumerate(sampled_batches):
                ...
                if self._fixed_base:
                    ...
                    imitation_loss = F.mse_loss(student_actions, sampled_teacher_actions)
                    residual_loss = F.mse_loss(residual_actions, sampled_teacher_actions - sampled_actions)
                    dagger_loss = imitation_loss + 0.5 * residual_loss
                print(f"dagger_loss: {dagger_loss.item()}")
                print(f"imitation_loss: {imitation_loss.item()}")
                # optimization step
                self.optimizer.zero_grad()
