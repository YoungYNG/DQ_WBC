...
class DAggerTrainer(Trainer):
    ...
    def single_agent_train(self) -> None:
        ...
        threshold_timestep = self.cfg.get("pretrain_timesteps", 5000)
        ...
        for timestep in tqdm.tqdm(range(self.initial_timestep, self.timesteps), disable=self.disable_progressbar):
            ...
            with torch.no_grad():
                ...
                stu_output = self.agents.act(student_obs, timestep=timestep, timesteps=self.timesteps)
                actions = stu_output[0]
                residual_actions = stu_output[1]
